# basically the nixos module but ported to home manager
# i should probably upstream this
# feel free to upstram this on you own
# TODO: actually finish this (and use it)
{
  config,
  lib,
  options,
...
with lib; let
  ...
  devices = mapAttrsToList (_: device:
    device
    // {
      deviceID = device.id;
    })
...
in {
  ...
  options = {
    services.syncthing = {
      ...
      # FIXME: since this now is a home manager module this will break, cant have
      #  multiple things on the same address
      guiAddress = mkOption {
        type = types.str;
        # is this a good replacement?
        # maby use localhost?
        # use this? default = "syncthing.${home.username}";
  ...
  */
  ...
  config = mkIf cfg.enable {
    # disabled by default so we can proably remove it :)
    /*
    ...
    */
    ...
    systemd.user.services = {
      ...
      syncthing = mkIf cfg.userService {
        ...
        serviceConfig = {
          ...
          */
          ...
          ExecStartPre = let
            strIfnNull = val: data: optionalString (val != null) data;
          in
            mkIf (cfg.cert != null || cfg.key != null)
            "+${
              pkgs.writers.writeBash "syncthing-copy-keys" (
                "install -dm700 ${cfg.configDir}"
                + (strIfnNull cfg.cert
                  "install -Dm400 ${toString cfg.cert} ${cfg.configDir}/cert.pem")
                + (strIfnNull cfg.key
                  "install -Dm400 ${toString cfg.key} ${cfg.configDir}/key.pem")
              )
            }";
