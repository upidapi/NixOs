...
class Part:
    ...
    @staticmethod
    def check_changes(args, target="HEAD"):
        has_changes = run_cmd(
            f"git diff {target} --cached"
        ).strip() != ""
    ...
    @staticmethod
    def update_inputs():
        Print.banner("Updating Flake Inputs")
...
class Command:
    def add_format_show(args, cmp_target):
        Part.format_files()
    ...
    def rebuild(args, cmp_target="HEAD"):  
        ...
        exit() # dont push to avoid breaking stuff
    ...
    def pull(args):
        ...
        run_cmd("git fetch")
        if not Part.check_changes("HEAD origin/main"):
            exit()
