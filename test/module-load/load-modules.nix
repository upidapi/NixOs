...
rec {
  ...
  recursiveMerge = with nixpkgs.lib;
    attr_list: let
      f = attr_path:
        zipAttrsWith (
          n: values:
  ...
  is-nix-file = name: kind:
    if is-file-kind kind
    then nixpkgs.lib.hasSuffix ".nix" name
    else false;
  ...
  */
  ...
  only-part = modules_data: thing:
    ...
    // (
      builtins.mapAttrs
      (
        _: sub_module_data: (only-part sub_module_data thing)
      )
  ...
  eval-module-file = {
    ...
    # dont try too call somehing that isn't a function
    modules_data =
  ...
  in
    if builtins.typeOf modules_data == "string"
  ...
  wrap-thing-with = vals: set:
    if builtins.length vals == 0
    then set
    else {
      "${builtins.head vals}" =
        wrap-thing-with (nixpkgs.lib.drop 1 vals) set;
    };
